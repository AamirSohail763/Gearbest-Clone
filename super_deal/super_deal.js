...
// cart count
  ...
  arr.forEach(function (product) {
    ...
    price.innerText = "$" + ((product.mrp *(100 - product.discount)) / 100).toFixed(2);
  ...
  if(sortType){
    ...
    for(var i =0;i<products.length-1;i++){
      for(var j = i+1;j<products.length;j++){
        Iprice = Number(products[i].mrp)*(100 - Number(products[i].discount))/100;
        Jprice = Number(products[j].mrp)*(100 - Number(products[j].discount))/100;
        if(Iprice > Jprice){
          var temp = products[i];
