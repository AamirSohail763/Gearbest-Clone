...
function displayCard(arr) {
  let cards = document.querySelector(".product_cards");
  ...
  arr.forEach(function (product) {
    ...
    let prod_txt_cont = document.createElement("div");
    ...
    // adding inner text or values

    imglink.href = "../description/description.html"; // page link ----------------------------
    let img = document.createElement("img");
    img.src = product.img;
    imglink.addEventListener("click", function () {
      localStorage.setItem("CurrProdDesc", JSON.stringify(product));
    });
    let disCir = document.createElement("div");

    disCir.setAttribute("class", "discount_cir");
    let disPer = document.createElement("span");
    disPer.setAttribute("class", "discount_per");
    disPer.innerText = product.discount + "% " + "OFF";
    disCir.append(disPer);

    prod_txt_cont.setAttribute("class", "product_text_container");
    ...
    plink.href = "../description/description.html"; //page link---------------------------
    pname.append(plink);
    pname.setAttribute("class", "product_name");

    plink.innerText = product.name.slice(0, 30) + "...";

    let priceCont = document.createElement("div");
    priceCont.setAttribute("class", "price_container");

    let price = document.createElement("span");
    price.setAttribute("class", "price");
    price.innerText =
      "$" + ((product.mrp * (100 - product.discount)) / 100).toFixed(2);
    priceCont.append(price);
    let strickPrice = document.createElement("span");
    strickPrice.setAttribute("class", "strickprice");
    strickPrice.innerText = "MRP: ";
    let del = document.createElement("del");
    del.innerText = "$" + product.mrp;
    strickPrice.append(del);
    priceCont.append(strickPrice);

    bottom_info.setAttribute("class", "bottom_info");
    let storeName = document.createElement("div");
    storeName.setAttribute("class", "storeName");
    bottom_info.append(storeName);

    let cartBag = document.createElement("div");
    cartBag.setAttribute("class", "cartBag");
    let cartLink = document.createElement("img");
    cartLink.setAttribute("class", "cart");
    cartLink.src =
      "https://uidesign.gbtcdn.com/GB/image/2019/20190417_9244/cart_default.png";
    cartBag.append(cartLink);
    cartLink.addEventListener("click", function () {
      addToCart(product);
    });

    bottom_info.append(cartBag);
    storelink = document.createElement("a");
    storelink.href = "#";

    let storeImg = document.createElement("img");
    storeImg.src =
      "https://uidesign.gbtcdn.com/GB/image/2019/20190417_9244/icon_store.png";
    let store = document.createElement("span");
    store.innerText = product.store;
    storelink.append(storeImg, store);
    storeName.append(storelink);

    prod_txt_cont.append(pname, priceCont, bottom_info);

    imglink.append(img);
    imgDiv.append(imglink, disCir);
    card.append(imgDiv, prod_txt_cont);
...
// -----------------------------------------------------
let sortByPrice = document.querySelector(".sort_price");

sortByPrice.addEventListener("click", function () {
  let cards = document.querySelector(".product_cards");
  if (cards.classList.contains("sort_price_asc")) {
    cards.classList.remove("sort_price_asc");
    cards.classList.add("sort_price_desc");
  } else {
    cards.classList.add("sort_price_asc");
  }
  let ascSort = cards.classList.contains("sort_price_asc");
  cards.innerHTML = "";
  sortPrice(products, ascSort);
});
// -----------------------------------------------------

let sortByDiscount = document.querySelector(".sort_discount");
sortByDiscount.addEventListener("click", function () {
  let cards = document.querySelector(".product_cards");
  if (cards.classList.contains("sort_dis_asc")) {
    cards.classList.remove("sort_dis_asc");
    cards.classList.add("sort_dis_desc");
  } else {
    cards.classList.add("sort_dis_asc");
  }
  let sort_type = cards.classList.contains("sort_dis_asc");
  cards.innerHTML = "";
  sort_discount(products, sort_type);
});

// ------------------------------------------------------

function sortPrice(products, sortType) {
  if (sortType) {
    for (var i = 0; i < products.length - 1; i++) {
      for (var j = i + 1; j < products.length; j++) {
        Iprice =
          (Number(products[i].mrp) * (100 - Number(products[i].discount))) /
          100;
        Jprice =
          (Number(products[j].mrp) * (100 - Number(products[j].discount))) /
          100;
        if (Iprice > Jprice) {
          var temp = products[i];
...
// discount sort Function

function sort_discount(products, sort_type) {
  if (sort_type) {
    for (var i = 0; i < products.length - 1; i++) {
      for (var j = i + 1; j < products.length; j++) {
        if (Number(products[i].discount) > Number(products[j].discount)) {
          var temp = products[i];
          products[i] = products[j];
          products[j] = temp;
        }
